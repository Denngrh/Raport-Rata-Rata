<template>
    ...
    <main class="xl:ml-[15.625rem] relative h-full max-h-screen rounded-xl transition-all duration-200">
        ...
        <nav class="relative flex flex-wrap items-center justify-between px-0 py-2 mx-6 transition-all shadow-none duration-250 ease-soft-in rounded-2xl lg:flex-nowrap lg:justify-start"
            ...
            <div class="flex items-center justify-between w-full px-4 py-1 mx-auto flex-wrap-inherit">
                <nav>
                    ...
                    <h6 class="mb-0 font-bold capitalize">Update</h6>
                </nav>
        ...
        <div class="w-full px-6 py-6 mx-auto">
            ...
            <form @submit.prevent="updateMapel">
                <!-- Mapel Input -->
                <div class="mt-10 card">
                    <div class="grid grid-cols-1 gap-x-6 gap-y-8 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4">
                        <div class="col-span-2 sm:col-span-1">
                            <label for="mapel" class="block text-sm font-medium leading-6 text-gray-900">Mata
                                Pelajaran</label>
                            <div class="mt-2">
                                <input type="text" v-model="mapel.mapel" autocomplete="given-name" class="input-field" />
                            </div>
                        </div>
                        ...
                        <div class="col-span-1">
                            <label for="kkm" class="block text-sm font-medium leading-6 text-gray-900">KKM (Kelulusan
                                Minimal)</label>
                            <div class="mt-2">
                                <input type="number" v-model="mapel.kkm" autocomplete="family-name" class="input-field" />
                            </div>
                        </div>
                        ...
                        <div class="col-span-1">
                            ...
                            <div class="mt-2">
                                <input type="number" v-model="mapel.nilai"
                        ...
                        <div class="sm:col-span-6 mt-4">
                            <button type="submit"
                                class="inline-flex items-center px-4 py-2 border border-transparent text-sm leading-5 font-medium rounded-md text-white bg-indigo-600 hover:bg-indigo-500 focus:outline-none focus:shadow-outline-indigo focus:border-indigo-700 active:bg-indigo-800 transition duration-150 ease-in-out">
                                Update
                            </button>
                        </div>
                    </div>
...
const updateMapel = async () => {
    await supabase
        .from('mapels')
        .update({
            mapel: mapel.value.mapel,
            kkm: mapel.value.kkm,
            nilai: mapel.value.nilai
        })
        .eq('id', mapel.value.id)
    router.push('/mapel/raport')
}
const getDataMapel = async (id) => {
...
.card {
    /* Tambahkan gaya khusus untuk kelas "card" di sini */
