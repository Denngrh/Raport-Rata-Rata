<template>
  ...
  <main class="xl:ml-[15.625rem] relative h-full max-h-screen rounded-xl transition-all duration-200">
    ...
    <nav
      ...
      <div class="flex items-center justify-between w-full px-4 py-1 mx-auto flex-wrap-inherit">
        <nav>
          ...
          <h6 class="mb-0 font-bold capitalize">Create</h6>
        </nav>
    ...
    <div class="w-full px-6 py-6 mx-auto">
      ...
      <form @submit.prevent="createNilai">
        <div class="border-b border-gray-900/10 pb-12">
          <h2 class="text-base font-semibold leading-7 text-gray-900"> Tambahkan Nilai</h2>
          <p class="mt-1 text-sm leading-6 text-gray-600">Masukan Nilai</p>

          <div class="mt-10 card">
            <div class="grid grid-cols-1 gap-x-6 gap-y-8 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4">
              <!-- Mapel Input -->
              <div class="col-span-2 sm:col-span-1">
                <label for="mapel" class="block text-sm font-medium leading-6 text-gray-900">Mata Pelajaran</label>
                <div class="mt-2">
                  <input v-model="mapel" type="text" autocomplete="given-name" placeholder="Contoh: Bahasa Inggris"
              ...
              <div class="col-span-1">
                <label for="kkm" class="block text-sm font-medium leading-6 text-gray-900">KKM (Kelulusan Minimal)</label>
                <div class="mt-2">
                  <input v-model="kkm" type="number" autocomplete="family-name" placeholder="Contoh: 75"
                    class="input-field" required />
                </div>
              </div>
              ...
              <div class="col-span-1">
                ...
                <div class="mt-2">
                  <input v-model="nilai" type="number" placeholder="Contoh: 85" class="input-field" required />
                </div>
              </div>
...
const createNilai = async () => {
  const { data, error } = await supabase
    .from('mapels')
    .insert([
      { mapel: mapel.value, kkm: kkm.value, nilai: nilai.value },
    ])
  if (error) {
